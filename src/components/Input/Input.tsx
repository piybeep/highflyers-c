...
export function Input({ placeholder, isBordered = false, isPlaceholder = false, isPassword = false, isEdit = false, inputType = 'text', ...props }: InputProps) {

    const [isType, setIsType] = useState(!isPassword)

    return (
        <div className={s.wrapper}>
            <p className={classNames(s.placeholder, {
                [s.placeholder__none]: !isPlaceholder
            })}>{placeholder}</p>
            <div className={s.wrapper__info}>
                <div className={s.info}>
                    <input className={classNames(s.input, {
                        [s.input__bordered]: isBordered
                    })} placeholder={placeholder} type={isPassword ? !isType ? 'password' : 'text' : inputType} />

                    <svg onClick={() => setIsType(false)} className={classNames(s.info__eye, {
                    ...
                    <svg onClick={() => setIsType(true)} className={classNames(s.info__eye, {
                        [s.info__eye_visible]: !isType,
                        [s.info__eye__none]: !isPassword
                ...
                <div className={classNames(s.edit, {
                    [s.edit__none]: isEdit
                })}>
                    <svg className={s.edit__svg} width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <rect width="20" height="20" rx="10" fill="#EEEEEE" />
                        <path fillRule="evenodd" clipRule="evenodd" d="M12.7521 5.81847C12.6126 5.679 12.3865 5.679 12.247 5.81847L6.93854 11.127C6.87429 11.1912 6.82288 11.2672 6.78709 11.3507L5.74275 13.7875C5.68523 13.9217 5.71522 14.0774 5.81848 14.1807C5.92175 14.2839 6.07748 14.3139 6.21171 14.2564L8.64848 13.2121C8.73201 13.1763 8.80793 13.1249 8.87219 13.0606L14.1807 7.75212C14.3202 7.61264 14.3202 7.38651 14.1807 7.24704L12.7521 5.81847ZM7.44362 11.632L12.4996 6.57609L13.4231 7.49958L8.36711 12.5555L7.29903 13.0133L6.98587 12.7001L7.44362 11.632Z" fill="#666666" />
                    </svg>
                </div>
            </div>
        </div>
