...
export function ListExams({ list }: ListExamsProps) {
    ...
    const arrayList = arrayGroup
        .map((currentGroup) => ({
            group: currentGroup.group,
            tag: currentGroup.tag,
            list: list
                .filter((current) => current.group === currentGroup.group)
            ...
            {arrayList.map((current) => (
                <div key={current.group} className={s.info}>
                    <div className={s.info__header} id={current.group}>
                        ...
                        <p
                            className={classNames(s.info__subtitle, {
                                [s.info__subtitle_write]:
                                    current.tag === 'Письменная часть',
                                [s.info__subtitle_verbal]:
