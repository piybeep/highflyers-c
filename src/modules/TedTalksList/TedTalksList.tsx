...
export function TedTalksList({ data }: { data: TedTalksProps[] }) {
    const arrayTheme = data
        .map((i) => i.theme)
        .filter(
            (currentTheme, index, currentArray) =>
                currentArray.indexOf(currentTheme) === index,
        );
    const arrayList = arrayTheme.map((currentTheme) => ({
        title: currentTheme,
        list: data.filter((current) => current.theme === currentTheme),
    }));
